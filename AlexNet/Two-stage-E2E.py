...
"""
experiment:
Two-stage training in centralized(E2E) manner
"""
...
normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]) # github上有数据loader配置
...
def train(model, optimizer, Tra_flag):
    ...
    for data, target in train_loader:
        ...
        val = get_information(branch_output)    # (train_batch_size)维 输入已经在cuda上
        ...
        if Tra_flag:
            ...
            loss = torch.mean((1 - sigmoid(threshold - val)) * sepa_criterion(output, target) + sigmoid(threshold - val) * sepa_criterion(
                branch_output, target))  # maybe onehot
        loss.backward()
        optimizer.step()
